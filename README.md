...
| **Admin-Panel** | Manuelles Auslösen per Button – einbindbar auf jeder CMS-Seite |
...
4. Admin-Panel auf einer CMS-Seite einbinden:
   ```
...
Einen beliebigen alphanumerischen String erzeugen (mind. 8 Zeichen), z.B.:
...
Eine Datei `{key}.txt` mit dem Key als einzigem Inhalt anlegen und in den Webroot hochladen:
...
| Endpunkt | URL |
|---|---|
| Standard (IndexNow) | `https://api.indexnow.org/indexnow` |
...
Das Admin-Panel via `{PLUGIN(indexnow|admin_panel)}` auf einer CMS-Seite einbinden (z.B. einer passwortgeschützten Admin-Seite).
...
- Admin-Panel mit Status-Feedback
